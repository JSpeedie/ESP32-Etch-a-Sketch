...
void spi_oled_reset(struct spi_ssd1327 *spi_ssd1327) {
    /* {{{ */
    gpio_set_level(static_cast<gpio_num_t>(spi_ssd1327->rst_pin_num), 1);
    vTaskDelay(100 / portTICK_PERIOD_MS);
    gpio_set_level(static_cast<gpio_num_t>(spi_ssd1327->rst_pin_num), 0);
    vTaskDelay(100 / portTICK_PERIOD_MS);
    gpio_set_level(static_cast<gpio_num_t>(spi_ssd1327->rst_pin_num), 1);
    vTaskDelay(100 / portTICK_PERIOD_MS);
    /* }}} */
...
void spi_oled_send_cmd(struct spi_ssd1327 *spi_ssd1327, uint8_t cmd) {
    ...
    gpio_set_level(static_cast<gpio_num_t>(spi_ssd1327->dc_pin_num), 0);
...
void spi_oled_send_data(struct spi_ssd1327 *spi_ssd1327, void * data, \
    ...
    /* {{{ */
    /* spi_device_acquire_bus(*(spi_ssd1327->spi_handle), portMAX_DELAY); */
    gpio_set_level(static_cast<gpio_num_t>(spi_ssd1327->dc_pin_num), 1);
...
void spi_oled_draw_image(struct spi_ssd1327 *spi_ssd1327, uint8_t *image) {
    ...
    /* When using DMA, we can send 32 rows at a time */
    /* Draw the image. 128 rows, sending 32 rows over the wire per transaction */
    ...
    /* /1* When we aren't using DMA, we are limited to 1 row at a time *1/ */
    /* /1* Draw the image. 128 rows, sending 1 rows over the wire per transaction *1/ */
